...
The API is designed to be simple while offering flexibility. For example, inviting a bot to a meeting:
...
The API includes features not explicitly defined in the routes such as:

- WebSocket connections for real-time audio streaming
- Persona management and selection
- Integration with Pipecat's voice AI processing
- Background bot lifecycle management

The join route does present a few options, such as persona selection.
